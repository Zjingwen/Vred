...
Listeners.prototype.checkInView = function(){
    ...
    if(this.$parent != window){
        var pRect = this.$parent.getBoundingClientRect();
        var _top = this.rect.top > pRect.top && pRect.bottom > this.rect.top,
            _left = this.rect.left > pRect.left && pRect.right > this.rect.left;
        inView = _top && _left;

    }else{
        var topBottom = this.rect.top < window.innerHeight * this.preLoad && this.rect.bottom > 0,
...
var Lazy = (function(){
    ...
    var bind = function(el, binding, vnode){
        var value = handleValue(binding.value);
    ...
    var update = function(el, binding){
        ...
        var existListener = find(ListenerQueue, el);
    ...
    var unbind = function(el) {
        if (!el) return;
        var existItem = find(ListenerQueue, el);
    ...
    function initListen(el, start) {
        for (var i = 0; i < options.ListenEvents.length; i++) {
            var event = options.ListenEvents[i];
